...
struct arduino {
  ...
  string score;//contien le score finale
}p;

//bdd -> zone
void update_zone_user(string q, string u){
   MYSQL *conn;
   char *server = IP;
   char *user = USER;
   char *password = PASS;
   ...
   string query = "UPDATE user SET zone='"+q+"',userID='"+u+"' WHERE id=1";
...
string get_sql(){
   ...
   if (mysql_query(conn, "Select * from user where id=1;")) {
      fprintf(stderr, "%s\n", mysql_error(conn));
      exit(1);
   }
...
void update_score(string q){
   ...
   if (!mysql_real_connect(conn, server,
	...
	cout << "avant mon score est :"<< q << endl;
...
void play(char *buf){
	...
	stringstream ss(buf);
   	string temp[4];//tab convert from vector
